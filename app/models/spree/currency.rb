...
class Spree::Currency < ActiveRecord::Base
  ...
  class << self
    ...
    def current(char_code = nil)
      @current = char_code.is_a?(Spree::Currency) ? char_code : where(:char_code => char_code).first unless char_code.nil?
    ...
    # In the parameters you can specify the locale you wish to convert FROM.
		# Usage: Spree::Currency.conversion_from_current(100, :locale => "da")
